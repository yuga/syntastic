...
let g:_SYNTASTIC_VERSION = '3.7.0-167'
...
function! s:BufEnterHook() abort " {{{2
    call syntastic#log#debug(g:_SYNTASTIC_DEBUG_AUTOCOMMANDS,
        ...
        \ ', &buftype = ' . string(&buftype))
    if &buftype ==# ''
        "if needs, comment out the following line
        call s:notifiers.refresh(g:SyntasticLoclist.current())
...
function! SyntasticMake(options) abort " {{{2
    ...
    if has_key(a:options, 'postfunc')
        ...
        call syntastic#log#debug(g:_SYNTASTIC_DEBUG_LOCLIST, "postfunc:", errors)
    endif
